...
- **一鍵專注/遊戲模式 (Focus Mode)**：倒數 5 秒後鎖定前景應用程式，自動抓出消耗資源的背景肥大軟體（Chrome、Discord 等），強制壓縮其記憶體並套用 Windows 11 的 **EcoQoS 效率模式** (降低 CPU 優先權)，為您的遊戲或高效工作榨出最大效能！
- **特定應用瘦身 (Targeted Trimmer)**：在設定介面提供即時的背景高耗能軟體排行榜，允許您單擊圖示精準針對單一失控的應用程式執行記憶體瘦身，而不影響全局系統。
